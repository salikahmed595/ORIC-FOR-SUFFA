...
function EvidenceScreen({ store }) {
  ...
  return (
    <Scroll>
      <InstructionBanner id="evidence">Upload proof documents for each KPI. Green means complete,
        yellow means more documents are needed.</InstructionBanner>
      ...
      <Card style={{ marginBottom:16 }}>
        <div style={{ fontFamily:EF.body, fontSize:12.5, color:E.ink, marginBottom:9 }}>
          <b style={{ fontFamily:EF.mono }}>{withAll} of {indicators.length}</b> indicators have all required documents</div>
        <ProgressBar value={withAll} max={indicators.length}/>
        <div style={{ fontFamily:EF.body, fontSize:11.5, color:E.ink2, marginTop:8 }}>{totalDocs} documents total · 24.6 MB used</div>
      </Card>
          ...
          <div key={k.code} style={{ background:'#fff', border:`1px solid ${E.border}`, borderRadius:14,
            ...
            <div onClick={()=>setOpen(expanded?null:k.code)} className="dsu-card-tap"
              ...
              <div style={{ flex:1, minWidth:0 }}>
                ...
                <div style={{ fontFamily:EF.body, fontSize:11.5, color:E.ink2, marginTop:3 }}>{k.docs.length} of {k.reqDocs.length} documents uploaded</div>
              </div>
              {k.docs.length>0 && <span style={{ minWidth:22, height:22, padding:'0 6px', borderRadius:11, background:E.yellow,
                color:E.maroonDark, fontFamily:EF.mono, fontWeight:700, fontSize:11, display:'flex',
                alignItems:'center', justifyContent:'center' }}>{k.docs.length}</span>}
              ...
              <div style={{ padding:'0 14px 14px', borderTop:`1px solid ${E.border}` }}>
                {k.docs.length>0
                  ? <div style={{ display:'flex', flexDirection:'column', gap:9, marginTop:12 }}>
                      {k.docs.map((f,n)=>(<FileItem key={n} file={f} onRemove={()=>store.toast('Document removed','info')}/>))}
                    </div>
                  : <div style={{ fontFamily:EF.body, fontSize:12.5, color:E.ink2, textAlign:'center', padding:'14px 0 8px' }}>No documents uploaded yet</div>}
                <div style={{ marginTop:12 }}>
                  <SecondaryBtn icon="cloud" onClick={()=>store.nav('entry',{code:k.code})}>Manage Evidence</SecondaryBtn>
                </div>
              </div>
            )}
          </div>
...
function ReportsScreen({ store }) {
  ...
  const [gen, setGen] = uS3(null); // null | 'loading' | report name
        ...
        <Scroll>
          ...
          <div style={{ border:`1px solid ${E.border}`, borderRadius:12, overflow:'hidden', background:'#fff' }}>
            ...
            <div style={{ padding:16 }}>
              ...
              <div style={{ marginTop:14 }}><ImgPlaceholder label="bar chart by section" h={120}/></div>
            </div>
          </div>
        </Scroll>
        <StickyBar>
          <div style={{ display:'flex', gap:10 }}>
            {[['download','Download'],['share','Share'],['print','Print']].map(([ic,l])=>(
              <button key={l} onClick={()=>store.toast(l+' started','success')} className="dsu-press"
                style={{ ...btnReset, flex:1, flexDirection:'column', gap:5, padding:'10px 0', borderRadius:12,
                  border:`1.5px solid ${E.maroon}`, justifyContent:'center' }}>
  ...
  return (
    <Scroll>
      {gen==='loading' && (
        <div style={{ position:'absolute', inset:0, background:'rgba(255,255,255,.96)', zIndex:30, display:'flex',
      ...
      {reports.map((r,n)=>(
        <Card key={n} style={{ marginBottom:12 }}>
          <div style={{ display:'flex', alignItems:'center', gap:13 }}>
            <span style={{ width:44, height:44, borderRadius:12, background:r.c+'15', display:'flex',
              alignItems:'center', justifyContent:'center', flexShrink:0 }}><Icon name={r.ic} size={22} color={r.c}/></span>
            <div style={{ flex:1, minWidth:0 }}>
              <div style={{ fontFamily:EF.display, fontWeight:600, fontSize:14, color:E.ink }}>{r.t}</div>
              <div style={{ fontFamily:EF.body, fontSize:12, color:E.ink2, marginTop:2, lineHeight:'16px' }}>{r.d}</div>
            </div>
            <button onClick={()=>generate(r.t)} className="dsu-press" style={{ ...btnReset, height:36, padding:'0 14px',
              borderRadius:10, background:E.maroon, color:'#fff', fontFamily:EF.display, fontWeight:600, fontSize:12.5,
              justifyContent:'center', flexShrink:0 }}>Generate</button>
          </div>
        </Card>
      ))}
    </Scroll>
...
function ProfileScreen({ store }) {
  ...
  const Section = ({ title, items }) => (
    <div style={{ marginBottom:18 }}>
      <div style={{ fontFamily:EF.display, fontWeight:600, fontSize:13, color:E.ink2, textTransform:'uppercase',
    ...
    <>
      <Scroll pad={0}>
        <div style={{ background:`linear-gradient(135deg, ${E.maroon}, ${E.maroonDark})`, padding:'24px 18px 26px',
          ...
          <div style={{ width:60, height:60, borderRadius:'50%', background:'rgba(255,255,255,.15)', border:'2px solid rgba(255,255,255,.3)',
            display:'flex', alignItems:'center', justifyContent:'center', fontFamily:EF.display, fontWeight:700, fontSize:22 }}>{u.initials}</div>
          <div>
            <div style={{ fontFamily:EF.display, fontWeight:700, fontSize:19 }}>{u.name}</div>
            <span style={{ ...pill('#fff',E.maroon), marginTop:6 }}>{u.roleLabel}</span>
            <div style={{ fontFamily:EF.body, fontSize:12, color:'rgba(255,255,255,.75)', marginTop:8 }}>{u.dept}</div>
          </div>
        </div>
        <div style={{ padding:16 }}>
          <Section title="Account" items={[
            { icon:'shield', label:'Change Password', onClick:()=>store.toast('Opens password form','info') },
            { icon:'eye', label:'Biometric Login', toggle:true },
          ]}/>
          <Section title="Notifications" items={[
            ...
            { icon:'cal', label:'New Assessment Period', toggle:false },
          ]}/>
          <Section title="Help" items={[
            { icon:'bulb', label:'How to Use This App', onClick:()=>store.toast('Opens guide','info') },
            { icon:'user', label:'Contact ORIC Office', onClick:()=>store.toast('oric@dsu.edu.pk','info') },
            { icon:'reviewCheck', label:'Switch to Reviewer Mode', onClick:()=>store.login('review') },
          ]}/>
          <div style={{ textAlign:'center', fontFamily:EF.body, fontSize:11, color:E.ink3, margin:'4px 0 16px' }}>DSU ORIC PMS v1.0.0</div>
          <button onClick={()=>setSheet(true)} className="dsu-press" style={{ ...btnReset, width:'100%', height:50,
            borderRadius:12, border:`1.5px solid ${E.error}`, color:E.error, justifyContent:'center', gap:8,
            fontFamily:EF.display, fontWeight:600, fontSize:15 }}>
            <Icon name="logout" size={19} color={E.error}/>Log Out</button>
        </div>
      </Scroll>
      <BottomSheet open={sheet} onClose={()=>setSheet(false)}>
        <div style={{ textAlign:'center', marginBottom:6 }}>
          <span style={{ width:48, height:48, borderRadius:'50%', background:E.error+'15', display:'inline-flex',
            alignItems:'center', justifyContent:'center' }}><Icon name="logout" size={24} color={E.error}/></span>
        </div>
        <div style={{ fontFamily:EF.display, fontWeight:700, fontSize:17, color:E.ink, textAlign:'center', marginBottom:6 }}>Log out of your account?</div>
        <div style={{ fontFamily:EF.body, fontSize:13, color:E.ink2, textAlign:'center', marginBottom:18 }}>Your drafts are saved. You can log back in anytime.</div>
        <div style={{ display:'flex', gap:10 }}>
  ...
  return (
    <button onClick={()=>setOn(!on)} style={{ ...btnReset, width:42, height:24, borderRadius:12,
      ...
      <span style={{ display:'block', width:20, height:20, borderRadius:'50%', background:'#fff',
        transform:on?'translateX(18px)':'translateX(0)', transition:'transform .2s', boxShadow:'0 1px 3px rgba(0,0,0,.3)' }}/>
    </button>
  );
}

Object.assign(window, { EvidenceScreen, ReportsScreen, ProfileScreen, Toggle });
